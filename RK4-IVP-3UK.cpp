...
double calcfunction1( double X , double Y , double Z)
{
    return(3*X + Y - Z);
}
...
int main()
{
    ...
    for(int i=1; i<=n ;i++)
    {
        k1 = h*calcfunction1(a,b,c);
        l1 = h * calcfunction2(a,b,c);

        k2= h * calcfunction1(a+(h/2),b+(k1/2),c+(l1/2));
        l2= h * calcfunction2(a+(h/2),b+(k1/2),c+(l1/2));
        
        k3= h * calcfunction1(a+(h/2),b+(k2/2), c+(l2/2));
        l3= h * calcfunction2(a+(h/2),b+(k2/2), c+(l2/2));

        k4 = h * calcfunction1(a+h,b+k3,c+l3);
